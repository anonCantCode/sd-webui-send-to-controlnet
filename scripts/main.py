			...
			if currentElement == "extras_tab" and tabId in ["img2img_gallery", "txt2img_gallery"]:
				with gr.Column():
					if tabId == "txt2img_gallery":
						with gr.Row():
							controlNetDropdownTxToTx = gr.Dropdown(choices=items, value=items[0:1], multiselect=True, label="ControlNet Unit t2t", elem_id="sendto_controlnet_dropdown_tx_to_tx")
						...
						with gr.Row():
							gr.Dropdown(choices=items, value=items[0:1], multiselect=True, label="ControlNet Unit t2t", elem_id="sendto_controlnet_dropdown_im_to_im")
